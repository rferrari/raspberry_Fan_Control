...
#########################
# rferrari:  minor changes for my raspberry setup
# 
...
# at 3.3V
#
# run-fan was tested on a raspberry pi running kodi on adam using a 3.3v
# GPIO
#
#########################
...
#   Group=root
#   Type=simple
#   ExecStart=/usr/bin/python /home/adam/run-fan.py
#   Restart=Always
#
#   [Install]
...
#fileLog = open('/home/adam/pixbee-rebase/app/logs/run-fan.log', 'wb+')
...
def printMsg(s):
    # we dont need file log, using pm2 log system, just print on screen and pm2 save it.
    #fileLog.write((timeStamp() + s + "\n").encode("utf-8"))
    print(s)

#########################
class Pin(object):
    pin = 14        # GPIO or BCM pin number to turn fan on and off
...
class Temperature(object):
    ...
    def __init__(self):
        ...
        # keep it under 50
        self.stopTemperature = self.startTemperature - 10.0

        printMsg("Fan ON at:  " + str(self.startTemperature))
        printMsg("Fan OFF at: " + str(self.stopTemperature))

    def getTemperature(self):
        res = os.popen('/usr/bin/vcgencmd measure_temp').readline()
        self.cpuTemperature = float((res.replace("temp=","").replace("'C\n","")))
        # debug current cpuTemp
        #print("cpuTemp: " + str(self.cpuTemperature))
...
try:
    ...
    printMsg("CTRL+C keyboard interrupt")
    # on exit keep cooler on
    myFan.setFan(1, True, myPin)
    # myPin.exitPin()
    #fileLog.close()
    ...
    myFan.setFan(1, True, myPin)
    #myPin.exitPin()    
    #fileLog.close()    # dont need fileLog anymore
